...
class Unit:
    ...
    def __init__(self, unit_type: str, weapon: str, armour: str, level: int, name=""):
        ...
        self.armour_cost = ARMOUR_DATA[unit_type][armour] # change
        self.unit_cost = unit_data["cost"] #change
...
# change
        self.cost = (self.weapon_cost + self.armour_cost + self.unit_cost) * (level**2)

        # print(f"unit: {unit_type}-{self.unit_cost}, level: {level}, weapon: {weapon}-{self.weapon_cost}, armour: {armour}-{self.armour_cost}, cost: {self.cost}")
...
def simulation(left_army: List[Unit], right_army: List[Unit], verbose=True, timeout = None):
    left_army = [u.clone() for u in left_army]
    right_army = [u.clone() for u in right_army]
    ...
    while(len(left_army) > 0 and len(right_army) > 0):
        ...
        for unit in attack_order:
            ...
            if(unit in left_army):
                ...
                enemy = left_army
                allies = right_army
            actual_range = unit.atk_range - (allies.index(unit))
            if(actual_range <= 0):
                sprint(f"{unit.name} couldn't attack because the idiot was out of range ", verbose)
                continue

            actual_range = min(actual_range, len(enemy))

            defender = enemy[random.randint(0, actual_range-1)]
            attacker = unit
